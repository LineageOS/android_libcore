...
sed -e '3i\' -e '<!-- generated by libcore/tools/update-lint-baseline/update-lint-baseline.sh -->' \
    -e 's/level 33/level 32/' -e '/issues>/ {d;q}' "${NEW_BASELINE}" > "${LIBCORE_BASELINE}"
...
sed -e '1,2d' -e '/issues>/ {d;q}' "${NEW_BASELINE}" >> "${LIBCORE_BASELINE}"

# Generate suppressions at API Level 34
sed -e '1,2d' -e 's/level 33/level 34/' "${NEW_BASELINE}" >> "${LIBCORE_BASELINE}"
