...
// Android-changed: stdio.h has to be included explicitly. Add some headers to get the boot time.
#include <errno.h>
#include <stdio.h>
#include <time.h>
#include <sys/sysinfo.h>

#define LOG_TAG "ProcessHandleImpl"
 ...
 */
static long long getBoottime(JNIEnv *env) {
   // BEGIN Android-changed: Re-implement using sysinfo. Normal Android app can't open `/proc/stat`.
   /*
    FILE *fp;
    char *line = NULL;
    size_t len = 0;
    ...
    */
    ...
    errno = 0;
    if (sysinfo(&si)) {
        ALOGE("failed to get bootTime_ms: %s", strerror(errno));
        // Terminates on a debug build.
        LOG_FATAL("failed to get bootTime_ms: %s", strerror(errno));
        return -1;
    }

    long long uptime = si.uptime;
    time_t current_time = time(NULL);

    return (current_time - uptime) * 1000;
   // END Android-changed: Re-implement using sysinfo. Normal Android app can't open `/proc/stat`.
}
