...
public class ScannerStreamTest extends OpTestCase {
    ...
    // Android-removed: reading from resources, not local files.
    ...
    // Android-added: reading from resources, not local files.
    Scanner makeResourceScanner() {
        var inputStream = ScannerStreamTest.class.getResourceAsStream("input.txt");
    ...
    public void findAllFileTest() {
        ...
        try (Scanner sc = makeResourceScanner()) {
            String match;
        ...
        Supplier<Stream<String>> ss =
            () -> makeResourceScanner().findAll(pat).map(MatchResult::group);
