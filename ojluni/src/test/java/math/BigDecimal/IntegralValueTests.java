...
public class IntegralValueTests {
    ...
    private static int reportError(BigDecimal bd, Number expected, long longValue, boolean isInt) {
        // Android-changed: Avoid conditional expression since JLS 15.25.2 promotes Integer to Long.
        // This patch can be upstreamed in the future.
        ...
        if (isInt) {
            System.err.format("For %s, scale=%d, expected %d, actual %d, simple %d%n",
                    bd.toString(), bd.scale(), expected, longValue, simpleIntValue(bd));
        } else {
            System.err.format("For %s, scale=%d, expected %d, actual %d, simple %d%n",
                    bd.toString(), bd.scale(), expected, longValue, simpleLongValue(bd));
        }
        return 1;
    }
