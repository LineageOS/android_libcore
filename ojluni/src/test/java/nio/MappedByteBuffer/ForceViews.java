...
public class ForceViews {
    ...
    // Android-changed: TestNgRunner recognizes this method as a test method.
    // @BeforeTest(alwaysRun=true)
    private void openChannel() throws IOException {
        // Android-changed: test.src is a read-only directory on Android.
        // Path file = Path.of(System.getProperty("test.src", "."), "junk");
        Path file = Path.of(System.getProperty("java.io.tmpdir", "."), ForceViews.class.getName());
    ...
    // Android-changed: TestNgRunner isn't able to run this parameterized test.
    // @Test(dataProvider = "provider")
    private void test(String tst, FileChannel fc, int mapPosition, int mapLength,
    ...
    // BEGIN Android-changed: TestNgRunner recognizes @BeforeTest and @AfterTest methods correctly.
    private void assertTest(int index) throws Exception {
        Object[] o = provider()[index];
        openChannel();
        try {
            test((String) o[0], fc, (int) o[2], (int) o[3], (int) o[4], (int) o[5], (int) o[6],
                    (int) o[7], (BiFunction<MappedByteBuffer, Segment, MappedByteBuffer>) o[8]);
        } finally {
            closeChannel();
        }
    }
    ...
    // END Android-changed: TestNgRunner recognizes @BeforeTest and @AfterTest methods correctly.
}
